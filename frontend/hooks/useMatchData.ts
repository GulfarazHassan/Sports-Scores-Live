  ...
  const handleWSMessage = useCallback((msg: WSMessage) => {
    switch (msg.type) {
      case "score_update":
        ...
        break;
      case "commentary": {
        const active = latestMatchIdRef.current;
        if (active == null) {
          return;
        }
        const mid = msg.data?.matchId;
        if (mid != null && String(mid) !== String(active)) {
          return;
        }
        const normalized = {
          ...msg.data,
